...
class yolov5_demo():
    ...
    # ----------------------------------------------
    def image_callback(self, image_raw, pub_bbox):
        class_list = []
        confidence_list = []
        x_min_list = []
        y_min_list = []
...
class yolov5_ros(Node):
    def __init__(self):
        ...
        self.pub_bbox_custom = self.create_publisher(BoundingBoxes, 'yolov5/bounding_boxes_custom', 10)  # 新しいトピックを追加
        self.pub_image_custom = self.create_publisher(Image, 'yolov5/image_raw_custom', 10)  # 新しいトピックを追加

        self.sub_image = self.create_subscription(Image, 'image_raw', self.image_callback,10)
        self.sub_custom_image = self.create_subscription(Image, 'custom_image_topic', self.custom_image_callback,10)  # 新しいトピックを追加
    ...
    def image_callback(self, image:Image):
        ...
        class_list, confidence_list, x_min_list, y_min_list, x_max_list, y_max_list = self.yolov5.image_callback(image_raw, self.pub_bbox)
        ...
        cv2.imshow("yolov5_image", image_raw)
        cv2.waitKey(1)

    def custom_image_callback(self, custom_image:Image):
        custom_image_raw = self.bridge.imgmsg_to_cv2(custom_image, "bgr8")
        class_list, confidence_list, x_min_list, y_min_list, x_max_list, y_max_list = self.yolov5.image_callback(custom_image_raw, self.pub_bbox_custom)

        msg = self.yolovFive2bboxes_msgs(bboxes=[x_min_list, y_min_list, x_max_list, y_max_list], scores=confidence_list, cls=class_list, img_header=custom_image.header)
        self.pub_bbox_custom.publish(msg)

        self.pub_image_custom.publish(custom_image)

        cv2.imshow("yolov5_custom_image", custom_image_raw)
        cv2.waitKey(1)
